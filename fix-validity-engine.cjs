...
function computePermitStatus(startDate, endDate) {
  ...
  if (!start || !end) {
    return {
      statusArabic: "غير محدد",
      statusEnglish: "Undefined",
      statusClass: "warning"
    };
  }
  ...
  if (start === tomorrow) {
    return {
      statusArabic: "يبدأ غدًا",
      statusEnglish: "Starts tomorrow",
      statusClass: "warning"
    };
  }
  ...
  if (start > tomorrow) {
    return {
      statusArabic: "لم يبدأ",
      statusEnglish: "Not started",
      statusClass: "warning"
    };
  }
  ...
  if (end === today) {
    return {
      statusArabic: "آخر يوم ساري",
      statusEnglish: "Last valid day",
      statusClass: "warning"
    };
  }
...
s = s.replace(
  /function normalizeDateOnly[\s\S]*?function computePermitValidity[\s\S]*?\n}\n/,
  validityBlock + "\n"
);

s = s.replace(
  /function computePermitValidity[\s\S]*?\n}\n/,
  validityBlock + "\n"
);

s = s.replace(
  /function mapPermitPayload\(permit, secureToken\) \{[\s\S]*?\n}\n/,
`function mapPermitPayload(permit, secureToken) {
  ...
  return {
    ...
    statusArabic: validity.computedStatusArabic || permit.statusArabic || "",
...
console.log("AZHA VALIDITY ENGINE FIXED: dynamic status now comes from dates.");
